{{-- resources/views/classroom/roles/admin/sections/show.blade.php --}}
<x-app-layout>
    <div class="max-w-6xl mx-auto px-4 py-6 space-y-4">
        ...
        >
            ...
            <x-breadcrumbs :items="[
                ...
                [
                    'label' => $section->instructor?->full_name ?? 'Courses',
                    'url' => route('classroom.admin.courses.index', [
                        'period_id'     => $section->academic_period_id,
            ...
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    >
                        ...
                        <span class="hidden sm:inline">View Attendance Photos</span>
                    </button>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 text-right whitespace-nowrap">
                                    <div class="inline-flex items-center space-x-2">
                                        ...
                                        {{-- Delete schedule --}}
                                        <form action="{{ route('classroom.admin.sections.schedules.destroy', [$section, $schedule]) }}"
                                              method="POST"
                                              class="inline"
                ...
                <div class="px-4 py-3 border-b border-slate-200">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap gap-2 justify-start sm:justify-end">
                            {{-- Add Student --}}
                            <button type="button" x-data x-on:click="$dispatch('open-modal', { name: 'add-student' })" class="inline-flex items-center gap-2 rounded-lg bg-[#0052CC] text-white text-xs font-medium px-3 py-2 shadow-sm hover:bg-[#003fa3]"> <span class="bi bi-plus-lg"></span> Add Student </button>
                            ...
                            {{-- Export CSV --}}
                            <a href="{{ route('classroom.admin.sections.students.export', $section) }}"
                                class="inline-flex items-center rounded-lg border border-slate-200 text-xs font-medium px-3 py-2 text-slate-700 hover:bg-slate-50">
                                Download Roster CSV
                            </a>
                        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($enrollments as $enrollment)
                            @php
                                ...
                                $stats   = $perStudentStats[$student->id] ?? null;
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    @else
                                        <form method="POST"
                                            action="{{ route('classroom.admin.sections.students.notify-face', [$section, $student]) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    ...
                                    <button
                                        type="button"
                                        x-on:click="openAttendance('{{ route('classroom.admin.sections.students.attendance', [$section, $student]) }}')"
                                    ...
                                    {{-- Remove from section --}}
                                    <form action="{{ route('classroom.admin.sections.students.destroy', [$section, $enrollment]) }}"
                                        method="POST"
                                        class="inline"
            ...
            >
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" x-on:click.stop>
                    ...
                    <form action="{{ route('classroom.admin.sections.schedules.store', $section) }}" method="POST" class="space-y-4">
                        @csrf
            ...
            @foreach ($section->schedules as $schedule)
                ...
                >
                    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" x-on:click.stop>
                        ...
                        <form action="{{ route('classroom.admin.sections.schedules.update', [$section, $schedule]) }}" method="POST" class="space-y-4">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select name="day_of_week"
                                    ...
                                    <option value="">Select day…</option>
                                    @foreach ($daysOfWeek as $value => $label)
                                        <option value="{{ $value }}" @isset($schedule) @selected($schedule->day_of_week == $value) @endisset>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
...
<div
    x-data="{
        ...
        choose(student) {
            this.query = student.student_number; // form sends pure student_number
            this.showList = false;
            this.focused = false;
...
>
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" x-on:click.stop>
        ...
        <form action="{{ route('classroom.admin.sections.students.store', $section) }}"
              method="POST"
              class="space-y-4">
            @csrf
                    ...
                    <template x-for="student in suggestions" :key="student.id">
                        ...
                        >
                            <span class="font-medium text-slate-900"
                                  x-text="student.student_number"></span>
                ...
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" x-on:click.stop>
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500 leading-snug">
                                Upload the CSV exported for this section. Only the <strong>Student Number</strong> column
                                should be edited. New numbers will be added; removed numbers will be unenrolled.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form action="{{ route('classroom.admin.sections.students.import', $section) }}"
                          method="POST"
...
>
    ...
    @php
        $hasPhotos = isset($photoGroups) && $photoGroups->isNotEmpty();
    @endphp
    ...
    >
        ...
        >
            ...
            {{-- Modal body --}}
            <div class="px-5 py-4 overflow-y-auto space-y-5 text-sm">
                @if (! $hasPhotos)
                    <div class="flex flex-col items-center justify-center py-10 text-center text-slate-400">
                        ...
                        <p class="text-sm">No attendance photos have been uploaded for this section yet.</p>
                    </div>
                @else
                    @foreach ($photoGroups as $date => $photos)
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                @foreach ($photos as $photo)
                                    @php
                                        $url = Storage::disk('r2')->url($photo->photo_path);
                                    @endphp
                                    <a
                                        href="{{ $url }}"
                                        target="_blank"
                                    ...
                                    >
                                        <div class="aspect-video rounded-lg overflow-hidden bg-slate-100">
                                            <img
                                                src="{{ $url }}"
                                                alt="Class attendance photo"
                                                class="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-150"
                                            >
                                        </div>
                                        <div class="mt-1 text-[11px] text-slate-500 flex items-center justify-between">
                                            <span>#{{ $photo->id }}</span>
                                            @if ($photo->created_at)
                                                <span>{{ $photo->created_at->format('g:i A') }}</span>
                                            @endif
                                        </div>
                                    </a>
                                @endforeach
    ...
    {{-- Alpine component for section page --}}
    <script>
