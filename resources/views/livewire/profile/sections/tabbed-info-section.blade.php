            ...
            <dl class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              <div>
                ...
                @if($viewerIsOwner ?? false)
                    <button
                        type="button"
                        wire:click="openStudentNumberReport"
                        class="mt-1 text-xs text-blue-600 hover:underline"
                    >
                        Report a problem with my student number
                    </button>
                @endif
              </div>
              <div>
...
{{-- REPORT STUDENT NUMBER MODAL --}}
<div wire:teleport="#modal-root">
  <div
    x-data="{ open:false }"
    x-cloak
    x-on:open-modal.window="
      if ($event.detail.name === 'student-number-report') open = true;
    "
    x-on:close-modal.window="
      if ($event.detail.name === 'student-number-report') open = false;
    "
    x-trap.noscroll="open"
    class="relative z-[2003]"
  >
    <div x-show="open" x-transition.opacity class="fixed inset-0 bg-black/40"></div>
    ...
    >
      ...
      >
        {{-- Header --}}
        <div class="sticky top-0 z-10 bg-white/90 backdrop-blur border-b px-4 sm:px-6 py-3 flex items-center justify-between">
          <div>
            <div class="text-[15px] font-semibold">Report Student Number Issue</div>
            <div class="text-xs text-gray-500 mt-0.5">
              Tell us your correct student number so an admin can verify the conflict.
            </div>
          </div>
          <button
            type="button"
        ...
        {{-- Body --}}
        <form wire:submit.prevent="submitStudentNumberReport">
          <div class="p-4 sm:p-6 space-y-4 text-sm">

            <div class="space-y-1">
              <label class="text-xs font-medium text-gray-700">
                Correct Student Number <span class="text-red-600">*</span>
              </label>

              <input
                type="text"
                wire:model.defer="reportStudentNumberInput"
                placeholder="e.g., 23IT0123"
                class="form-input w-full text-sm"
              />

              @error('reportStudentNumberInput')
                <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
              @enderror
            </div>

            <p class="text-[11px] text-gray-500">
              We use this only to notify administrators. This will not automatically change your student information.
            </p>
          </div>

          {{-- Footer --}}
          <div class="sticky bottom-0 bg-white/95 backdrop-blur border-t px-4 sm:px-6 py-3 flex justify-end gap-2">
            <button
              type="button"
              class="h-8 px-3 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-50 text-xs"
              x-on:click="open = false"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              class="h-8 px-3 rounded-md bg-blue-600 text-white hover:bg-blue-700 text-xs font-medium disabled:opacity-60"
              wire:loading.attr="disabled"
              wire:target="submitStudentNumberReport"
            >
              <svg wire:loading wire:target="submitStudentNumberReport"
                   class="w-3.5 h-3.5 animate-spin inline mr-1"
                   viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <circle class="opacity-25" cx="12" cy="12" r="10"
                        stroke="currentColor" stroke-width="3"/>
