{{-- resources/views/livewire/admin/departments-page.blade.php (example name) --}}
<div>
        ...
        <div class="px-4 sm:px-5 py-3 border-b border-slate-200 dark:border-slate-700 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="space-y-0.5">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Manage academic departments and their active status.
                </p>
            </div>

            <div class="flex flex-col sm:flex-row gap-2 sm:items-center w-full sm:w-auto">
                <div class="w-full sm:w-64">
                    <label class="sr-only" for="department-search">Search departments</label>
                    <div class="relative">
            ...
            <div class="px-4 sm:px-5 pt-3 pb-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
                <div>
                    @if($rows->total() > 0)
                        ...
                        of <span class="font-semibold">{{ $rows->total() }}</span> departments
                    @else
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @foreach ($rows as $row)
                            ...
                            >
                                <td class="px-4 py-2 align-middle font-mono text-xs text-slate-800 dark:text-slate-100">
                                    {{ $row->abbrev }}
                                </td>
    ...
    <div wire:teleport="#modal-root">
        <div
            x-data="{ open:false }"
            x-cloak
            x-on:open-modal.window="if($event.detail.name==='department-modal') open=true"
            x-on:close-modal.window="if($event.detail.name==='department-modal') open=false"
